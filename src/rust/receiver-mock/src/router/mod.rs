...
impl AppState {
    pub fn add_traces_result(&self, result: traces::TracesHandleResult, _opts: &options::Options) {
        self.spans.fetch_add(result.spans_count, std::sync::atomic::Ordering::Relaxed);

        {
            let mut spans = self.spans_list.write().unwrap();
            let mut traces = self.traces_list.write().unwrap();
            for span in result.spans {
                traces.entry(span.trace_id.clone()).or_insert(traces::Trace::new()).span_ids.push(span.id.clone());
